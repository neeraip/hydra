...
and iterated to the same $10^{-4}$ tolerance over at most 40 passes. Only above $\alpha = 0.04$ does the circle revert to table lookup, so the near-empty regime every dynamic-wave step visits is analytic.
