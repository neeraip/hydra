...
3. **Explore results.** After the simulation completes, the network map updates with colour-coded results. Click any node or link to inspect its time-series values (pressure, head, flow, velocity, water age, etc.). Use the timeline scrubber to step through reporting periods. A drainage model with a 2D mesh also shows its surface on the map. The mesh itself is drawn from the moment the model opens, coloured by ground elevation, so you can read the terrain and see where water will collect before running anything; the Overview page counts its cells. Zoom in far enough for the cells to be told apart and their edges are drawn too, which is how you check where a mesh is refined. A large mesh viewed whole shows the surface without them, because every edge drawn at that size is a dark wash rather than a picture of anything. After a run the cells are coloured by water depth (or water surface, speed, or back to the ground, from the legend's picker), dry cells transparent, stepping with the same timeline. Hovering a cell reads out its value, and a toolbar button toggles the layer. Results are painted one flat colour per cell, because a cell is what the solver computes. The ground is different: a mesh stores an elevation at every cell corner, so the control beside the animation button offers two readings of the same terrain rather than two shadings of one. Flat paints each cell the mean of its three corners, which is what the solver's default closure works from; smooth interpolates between the corner elevations themselves, which is what the model was authored with, and lets the pointer read the elevation where it actually is. On steep terrain the two can differ by two thirds of a cell's relief.

2D is the newest part of the drainage engine, and what the canvas shows is deliberately conservative: the values the engine computed, and nothing between them.
