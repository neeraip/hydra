...
pub use crate::engine_api::analysis::source::{
    AnalyticsScan, OutEnergy, OutMetadata, OutValidityError, OutValidityKind, PeriodResult,
    PumpEnergyRecord, ResultRanges, ResultsSource,
};

/// The `.out` byte layout of a results set: format knowledge, kept on
/// the dialect side as an extension of the engine's neutral metadata.
pub trait OutLayout {
    fn prolog_bytes(&self) -> u64;
    fn energy_bytes(&self) -> u64;
    fn dynamic_offset(&self) -> u64;
    fn period_bytes(&self) -> u64;
}

impl OutLayout for OutMetadata {
...
/// A `.out` results file as a [`ResultsSource`]: metadata read at
/// open, everything else seeked from the path on demand — the
...
impl OutFileSource {
    ...
    pub fn open(path: &std::path::Path) -> Result<OutFileSource, OutValidityError> {
        let meta = read_metadata_checked(path)?;
        Ok(OutFileSource {
