...
The engine exposes a session API (§8.3) through which a caller can load a validated `Network`, drive the simulation, retrieve results, and serialize output. Model-file parsing is owned by the engine's I/O layer (`../model/spec.md` §4). The engine performs no filesystem or network I/O; callers supply bytes and receive structured results.

### 8.1 Input Contract

Model-file bytes are parsed by the engine's I/O layer (`../model/spec.md` §4), which performs format detection, parsing, unit conversion, and validation. The session receives a `Network` via `load()`.
...
```text
// ── Parsing (owned by the engine's I/O layer, not the session) ──
network = parse(bytes) // from the engine's I/O: format detection, conversion, validation
...
The engine provides a deterministic runtime estimator for hydraulic +
