...
| `speed_pattern` | Optional pattern ID supplying the speed setting each period — its multipliers **are** the speeds and replace `init_setting` ([simulation spec](../simulation/spec.md) §5.4) | nullable |
...
- `[LEAKAGE]`: the on-disk values are the FAVAD coefficients C₁ (mm² of fixed leak area per **100 feet** of pipe) and C₂ (**mm² of leak-area expansion per metre of pressure head**, per 100 feet of pipe). The per-100-feet basis holds in every unit system — it is defined by EPANET's runtime, whose internal pipe length is always feet — and both coefficients are areas in mm², so both convert by $10^{-6}$. At load they become the per-pipe discharge coefficients $K_1 = C_d\sqrt{2g} \cdot 10^{-6} \cdot C_1 \cdot L/30.48$ and $K_2 = C_d\sqrt{2g} \cdot 10^{-6} \cdot C_2 \cdot L/30.48$ (with $C_d = 0.6$, $L$ in metres, so $L/30.48$ is the pipe's count of 100-foot sections; §2.6.2, §2.10); the writer inverts these formulas. These match EPANET's leakage model $Q = C_d\sqrt{2g}\,(A_0 + m\,H)\sqrt{H}$ exactly, which its own solver enforces at convergence.
...
Two link-variable conventions follow EPANET's writer, which defines this format:

- **Pipe setting** is the roughness coefficient. Hazen-Williams $C$ and Manning $n$ are written as stored; Darcy-Weisbach roughness is written in **feet in every unit system** — EPANET emits its internal ft-based value raw, without unit conversion, so a `.out` consumer reads that column as feet regardless of the model's unit system.
- **Friction factor** is derived from the actual head drop, $f = \pi^2 g D^5 \, |\Delta h| / (8 L Q^2)$, for open pipes with $|Q|$ above the reporting cutoff; it is written as **0 for closed pipes**, for links that are not pipes, and below the cutoff. Without the closed-pipe zero, the residual seepage flow a closed pipe carries turns the formula into meaningless multi-million values.
