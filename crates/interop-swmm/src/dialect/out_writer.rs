...
/// A §14.9 results file written as the run produces it.
///
/// The whole-run [`write_out`] below is this writer driven to completion,
/// so the two cannot disagree about a byte. Streaming matters because the
/// alternative is holding every reporting instant of a run in memory,
/// which on a long one is the largest thing the engine owns and grows
/// without bound in the run's length.
///
/// Only [`Write`] is needed, never [`std::io::Seek`]: this format closes
/// with its section offsets rather than opening with them, so nothing has
/// to be gone back and patched.
...
impl<W: Write> OutStream<W> {
    /// Write everything that precedes the first period.
    ///
    /// `first_report_t` is the run time of the first reporting instant.
    /// The header needs it before any period exists, because §14.9
    /// backdates the stored start by one period so that a reader
    /// computing `start + n·step` lands on the true record times.
    pub fn begin(
        mut sink: W,
        net: &Network,
        start_epoch: f64,
        ...
        let (cv, subs, nodes, links, np, id_start, input_start, output_start) = {
            let w = &mut sink;
            let cv = Cv {
            ...
            let first_snap_epoch = start_epoch + first_report_t;
            ...
            (cv, subs, nodes, links, np, id_start, input_start, pos)
        };
        Ok(OutStream {
            w: sink,
            cv,
    ...
    pub fn append(&mut self, snap: &Snapshot) -> io::Result<()> {
        ...
        let (np, start_epoch) = (*np, *start_epoch);
    ...
    /// Close the file with the offsets a reader seeks back for, and give
    /// the sink back.
    pub fn finish(mut self) -> io::Result<W> {
        let w = &mut self.w;
        put_i32(w, self.id_start as i32)?;
        put_i32(w, self.input_start as i32)?;
        put_i32(w, self.output_start as i32)?;
        put_i32(w, self.periods)?;
...
pub fn write_out(
    ...
    // An empty run still records a first instant one step in, which is
    // what the header backdates from.
    let first = snapshots.first().map_or(report_step, |s| s.t);
    let mut out = OutStream::begin(w, net, start_epoch, report_step, first)?;
