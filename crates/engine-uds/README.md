...
The urban drainage engine (`uds`) of [Hydra](https://github.com/neeraip/hydra), the water infrastructure simulation platform. It simulates stormwater and wastewater collection networks on the SWMM data model: rainfall-runoff hydrology (Horton / Green-Ampt / Curve Number infiltration, LID controls, snowmelt, groundwater, RDII), Preissmann-slot dynamic-wave routing, pollutant buildup / washoff / treatment / transport, rule-based controls, and SWMM-compatible INP import and OUT/RPT output.

It is one of Hydra's domain engines, alongside water distribution (`hydra-engine-wds`).
